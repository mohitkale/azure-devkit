...
- Removed personal email addresses from distributed plugin metadata, privacy information, and license notice.
- Added a Marketplace catalog so GitHub users can install Azure DevKit through Claude Code's supported marketplace flow.
- Reworked the README as a user-focused product guide with Windows and macOS installation paths, safety boundaries, and test evidence.
- Made AKS credential retrieval permission-gated and removed the kubeconfig overwrite instruction from automatic diagnostic permissions.
- Added post-tool hook tests and a Docker Desktop test command that makes no Azure calls.

### Fixed

- Full audit examples no longer claim last-use or credential-age findings that the command does not collect.
