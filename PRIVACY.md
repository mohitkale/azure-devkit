# Privacy policy: azure-devkit (Claude Code plugin)

**Last updated:** 2026-04-19  

**Repository:** https://github.com/mohitkale/azure-devkit  

**Maintainer:** Azure DevKit maintainers
...
A developer plugin for **Microsoft Azure** workflows (for example Bicep, AKS, App Service, Azure Functions, and RBAC guidance).
...
**Session hooks:** Optional Node.js scripts under `hooks/` may read files in your **current working directory** (for example to detect Azure-related project markers) and emit short text as **additional context** inside your Claude Code session. That runs locally where Claude Code is installed. This plugin does not send that output to a separate server operated by the plugin authors.
...
For support, security-sensitive reports, or privacy questions, use the repository's GitHub issue tracker or its private reporting channel when one is configured.
