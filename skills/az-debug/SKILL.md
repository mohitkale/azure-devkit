...
allowed-tools: Bash(az aks show *) Bash(az aks list *) Bash(az aks nodepool list *) Bash(az aks nodepool show *) Bash(az aks check-acr *) Bash(az webapp log *) Bash(az webapp show *) Bash(az webapp list *) Bash(az functionapp show *) Bash(az functionapp list *) Bash(az monitor log-analytics query *) Bash(az monitor metrics list *) Bash(az monitor activity-log list *) Bash(az account show *) Bash(kubectl get *) Bash(kubectl describe *) Bash(kubectl logs *) Read Grep
---
...
Getting credentials changes the user's local kubeconfig and must be approved explicitly. Do not use `--overwrite-existing`; the default merge behavior preserves any existing entry. After approval, confirm the active context before inspecting the cluster.
